      ...
      <span class="title-line">
        ...
        >
          ...
          {#each rotatingWords as word, i}
            <span class="rotator-word" class:is-active={i === activeIndex} aria-hidden={i !== activeIndex}>
              {word}
            </span>
          {/each}
        </span>
...
<style>
  ...
  .hero-glow {
    ...
    width: 90%;
    height: 70%;
    background: radial-gradient(ellipse at center, rgba(102, 126, 234, 0.14) 0%, rgba(8, 145, 178, 0.06) 40%, transparent 70%);
    pointer-events: none;
  }
